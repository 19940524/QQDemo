...
//        NSLog(@"badgeLabelRc = %@",NSStringFromCGRect(badgeLabelRc));
...
- (void)layoutSubviews{
    ...
    if (_badgeLabel.text.length == 2) {
        _badgeLabel.frame = CGRectMake(_badgeLabel.frame.origin.x-10, _badgeLabel.frame.origin.y,_badgeLabel.frame.size.width+10, _badgeLabel.frame.size.height);
        _startCenterPoint = CGPointMake(_startCenterPoint.x-5, _startCenterPoint.y);
    }
//    if (_badgeLabel.text.length == 2 && _badgeLabel.text.integerValue != 99) {
//        _badgeLabel.frame = CGRectMake(_badgeLabel.frame.origin.x-10, _badgeLabel.frame.origin.y, _badgeLabel.frame.size.width+10, _badgeLabel.frame.size.height);
//    } else if (_badgeLabel.text.integerValue == 99) {
//        _badgeLabel.frame = CGRectMake(_badgeLabel.frame.origin.x-15, _badgeLabel.frame.origin.y, _badgeLabel.frame.size.width+15, _badgeLabel.frame.size.height);
//        _badgeLabel.text = @"99+";
//    }
}
            ...
            if(_enableDragBadgeLabel){
                ...
                if(_isBreak){
                    if(_validAttachDistance >= circleDistance){
                        ...
                        //销毁标签
                        [self removeSelf];
                        if (_delegate && [_delegate respondsToSelector:@selector(whc_BadgeViewDidRemoveFromSuperViewWithIndex:center:)]) {
                            CGPoint point = [panGesture locationInView:panGesture.view];
                            [_delegate whc_BadgeViewDidRemoveFromSuperViewWithIndex:_index center:point];
                        }
                    }
                }else{
                    [self handleEndDragAnimation:circleDistance];
